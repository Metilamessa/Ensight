...
export default function RecentArticles({
  ...
  useEffect(() => {
    ...
    recentAnalysis.forEach((article) => {
      if (article.image instanceof File) {
        urls[article.id] = URL.createObjectURL(article.image);
      } else if (article.image) {
        urls[article.id] = article.image;
      } else {
        urls[article.id] = "/images/logo-red.png"; 
      }
    });
    setImageUrls(urls);
      ...
      <div className="space-y-4">
        {recentAnalysis && recentAnalysis.length > 0 ? (
          recentAnalysis.map((article) => {
            const imageSrc = imageUrls[article.id] || "/images/logo-red.png";
